...
-- CONSTANTS
EXCLUDE_OPTIONS = {["others"] = '97', ["all"] = '98', ["none"] = '99'}
MAIN_QUESTION_TYPES = {"CHECKBOX", "RADIO", "TABLE", "RANK"}
others_text = "other"

------------------------------------

function sandbox(source_id, target_id) return end
...
function pipe_mcq_to_mcq(source_id, target_id)
    ...
    if not pipe_from_others then
        local target_title = getquestionoptions(target_id, "Title")
    end
    ...
    if source_type == "table" then
        ...
        local source_title = strip(getvaluelabel(source_id)) or
                                 print("ERROR CODE 0.2")
    ...
    for key, reporting_value in pairs(target_options) do
        ...
        if in_array(reporting_value, source_answer) then
            ...
            if reporting_value ~= EXCLUDE_OPTIONS["none"] then
                if not (pipe_from_others) and reporting_value ==
                    EXCLUDE_OPTIONS["others"] then
                    break
                else
        ...
        -- In case of others
        if ((pipe_from_others and reporting_value == EXCLUDE_OPTIONS["others"] and
            others_answer ~= nil) -- This case is if its piped from previous question
        or
            (not pipe_from_others and reporting_value ==
                EXCLUDE_OPTIONS["others"])) then -- This case is if respondent needs to input the others
            hideoption(target_id, reporting_value, false)
        end
...
function pipe_mcq_to_grid(source_id, target_id)
    ...
    if (type(source_id) ~= 'number' or type(secondary_source_id) ~= 'number') and
        type(target_id) ~= 'number' then return end

    others_answered = false
    local target_options = array_flip(gettablequestiontitles(target_id)) or
    ...
    for row_title, row_id in pairs(target_options) do
        if pipe_row then
            ...
            if in_array(row_title, src_ans_label) or
                (others_answered and row_title:find(strip("option value"))) then
...
function pipe_grid_to_grid(source_id, target_id)
    ...
    local source_title = array_flip(gettablequestiontitles(source_id)) or
                             print("ERROR CODE 2.2")
    local target_title = array_flip(gettablequestiontitles(target_id)) or
                             print("ERROR CODE 2.3")

    for key, row_id in pairs(target_title) do
        sa_title = source_answer[source_title[key]] or print("ERROR CODE 2.4"..key)
        hidequestion(row_id, true)
...
function pipe_grid_to_mcq(source_id, target_id)
    ...
    local source_title = array_flip(gettablequestiontitles(source_id)) or
                             print("ERROR CODE 2.2")
    local target_title = getquestionoptions(target_id, "Reporting") or
                             print("ERROR CODE 2.3")
    ...
    for k, v in pairs(ans) do
        if string.find(k, others_text) then
            for m, n in pairs(label) do
                if v == tostring(m) then label[m] = nil end
            end
        end
    end
...
function page_maker()
    ...
    for line in PAGES_ID:gmatch("(.-)\n") do
        ...
        if page_title == "current" then
            pages[page_title] = currentpagesku()
...
function multi_compare(c, ...)
    ...
    if c then
        if ans >= arg[2] and ans <= arg[3] then rst = true end
    else
        for i = 2, (#arg + 2) do
            if ans == arg[i] then
                rst = true
...
function recurs_merge(...)
    local arr = {}
    for i = 1, #arg do arr = array_merge(arr, arg[i]) end
    return arr
end
