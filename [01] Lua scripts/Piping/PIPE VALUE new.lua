...
function pipe_mcq_to_grid(source_id,target_id)
    ...
    for row_title,row_id in pairs(target_options)do
        ...
        if pipe_row then
			...
			-- if others_answered then print"others answered" end 
			-- if current_val == others_pos then print(row_title) print('current val: '..current_val) print('others_pos: '..others_pos)end
...
function source(id)
    ...
    for key,value in pairs(label) do
        ...
        if key == 97 or string.find(label[key],others_text) or string.find(label[key],'अन्य') or string.find(label[key],'ਹੋਰ') then
			label[key] = nil
